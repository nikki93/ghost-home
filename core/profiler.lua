local piefiller -- This gets set later, see below


local Profiler = core.entity.newComponentType('Profiler', {
    depends = { 'Input', 'Visual' },
})

function Profiler:add()
    self.pie = piefiller:new()
    self.Input.enabled = true
    self.Visual.depth = 2000
end
...
-- Code to profile a code segment:
--
--   local profilerEnt = next(core.entity.componentTypes.Profiler:getAll())
--   if profilerEnt then profilerEnt.Profiler.pie:attach() end
--   ...
--   if profilerEnt then profilerEnt.Profiler.pie:detach() end
--
-- Code to create a `Profiler`:
--
--   core.entity.new {
--       Profiler = {},
--   }
...
-- Below is from https://github.com/Polynominal/Piefiller/blob/master/piefiller.lua
